...
router.post("/logout", fetchuser, async (req, res) => {
  try {
    // You can access the user information using req.user if fetchuser middleware is used
    const loggedOutUser = await User.findById(req.user.id)

    // Simply send a success message indicating logout
    res.send({ success: true, message: `User ${loggedOutUser.name} logged out successfully` });
  } catch (error) {
    console.error(error.message);
    res.status(500).send("Internal server error");
  }
